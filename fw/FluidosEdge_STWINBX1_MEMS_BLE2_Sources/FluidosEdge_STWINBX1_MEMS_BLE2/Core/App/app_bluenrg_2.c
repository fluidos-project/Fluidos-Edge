...
uint8_t Sensor_DeviceInit(void)
{
  ...
  ret = aci_hal_read_config_data(config_data_stored_static_random_address, &bdaddr_len_out, bdaddr);
...
//      srand(HAL_GetTick());
      ...
      if (send_env) {
        /* Update MEMS Environmental data */
//        Set_Random_Environmental_Values(&data_t, &data_p);
    	  Get_MEMS_Data(0, &env_data);
